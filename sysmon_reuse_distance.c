...
int page_counts 			= 0;	// Records no. of used pages
...
static int scan_pgtable(void)
{
	...
	int i             = 0;
	int times 				= 0;	// Records Re-Use time
  ...
  else
  {
    ...
    for (cycle_index = 0; cycle_index < ITERATIONS; cycle_index++)
    {
      ...
      num_vpages 				= 0;
			sampling_interval = page_counts / 250;
			page_counts 			= 0;
          ...
          if (pte_present(pte))
          {
            if (pte_young(pte))
            {
              ...
              hot_pages[cycle_index]++;
							...
							{
								times++;
								if (pte_dirty(pte)) dirty_page[cycle_index] = 1;
							}
            }
						else
						{
